...
function getrfnl(src_path, dest_path)
...
if isUseExampleImage    
    ...
    %imagePath{1} = [folderPath '/home/rajiv/Desktop/fyp/formasking/Image 7.png'];
    fil_list=dir(src_path);
    for i=3:length(fil_list)
       imagePath{i-2}=[src_path fil_list(i).name]; 
       %display(i)
       %display(imagePath);
    end
...
for i = 1:numel(imagePath)
    ...
    display(sprintf('segmenting image %d of %d,imagePath{%d}',i,numel(imagePath),i));
    %display(imagePath{i})
    % read in the image.
    %disp(imagePath{i});
...
saveas(gcf, sprintf('%s%s.png', dest_path, fil_list(i+2).name(1:length(fil_list(i+2).name)-4)), 'png');
% % save segmentation
 %filename = [imageLayer(i).imagePath '_octSegmentation.mat'];
